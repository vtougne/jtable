            ...
            with open(args.query_file, 'r') as file:
                ...
                if 'facts' in query_set:
                    ...
                    for key,value in query_set['facts'].items():
                        ...
                        dataset = {**self.dataset,**facts}
                    self.dataset = facts
        
        out = JtableCls(render=render).render_object(self.dataset,path=self.path,select=select,vars=vars)[format]
        print(out)
        ...
        if self.render == "jinja_ansible":
            ...
            try:
                ...
                if str(error)[0:30] == "'dict object' has no attribute":
                    ...
                    out = error
                    logging.error("[ERROR] error while jinja_ansible rendering value, error was: " + str(error))
        else:
            try:
