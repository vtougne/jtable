...
def ansible_filter(dataset,select=[]):
    return JtableCls( render="jinja_ansible").render_object(dataset,path=path,select=select)
...
class JtableCli:
    ...
    def parse_args(self):
        ...
        if args.query_file:
            with open(args.query_file, 'r') as file:
                ...
                if 'vars' in query_set:
                    vars = query_set['vars']
                    # for key, value in query_set['vars'].items():
                ...
                if 'facts' in query_set:
                    ...
                    for key,value in query_set['facts'].items():
                        jinja_eval = JtableCls().jinja_render_value(str(value),facts)
                ...
                #     out = query_set['out']
                # else:
                #     out = "{{ stdin | jtable(select=select) }}"
        ...
        if args.json_files:
            ...
            if args.json_files[0] != "{":
                ...
                for file_name in files_str.split('\n'):
                    if file_name != '':
                        with open(file_name, 'r') as input_yaml:
                            file_content = { "file_content": yaml.safe_load(input_yaml)}
                            file_full_path = {"file_full_path": file_name}
                            file_list_dataset = file_list_dataset + [{ **file_full_path, **file_content }]
        ...
        if args.json_path:
            ...
            # if path is not terminated by {} or {something} then adding {} at the end
        ...
        #         shell_context = "tty"
        #     elif ret == 33188:
        #         shell_context = "redirected"
        #     elif ret == 4480:
        #         shell_context = "piped"
        #     else:
        #         shell_context = "unknown"

        #     if shell_context == "piped":
        #         out_splitted = out.split("\n")
        #         sys.stderr.write("\n".join(out_splitted[0:2]) + "\n" )
        #         print("\n".join(out_splitted[3:]) )
        #     else:
        #         print(out)
        # else:
...
class JtableCls:
    ...
    def render_object(self,dataset,path=path,select=[],vars={}):
    ...
    def jinja_render_value(self,template,context):
        if self.render == "jinja_ansible":
            ...
            tenv.filters['is_list'] = is_list_filter
            ...
            try:
                ...
                if str(error)[0:30] == "'dict object' has no attribute":
                    ...
                    # sys.stderr.write('debug self.dataset: ' + str(self.dataset))
                    # print('debug self.th: ' + str(self.th))
                    # print(tabulate(self.td,self.th))
                    
                    sys.stderr.write("[ERROR] error while jinja_native rendering value, error was: " + str(error) + "\n" )
                    exit(1)
            if out_str == "":
                out = None
            else:
