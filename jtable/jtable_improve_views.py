        ...
        for item in dataset_to_cover:
            ...
            def when(when=[],context={}):
                ...
                for condition in when:
                    ...
                    for var_name,var_data in self.views.items():
                        ...
                        logging.debug(f"view_context, var_name: {var_name}, templated_var: {templated_var}")
                    condition_template = Templater(template_string=jinja_expr, static_context= {**self.context,**context,**view_context})
                    condition_test_result = condition_template.render({},eval_str=True)
                    logging.info(f"condition_test_result: {condition_test_result}, jinja_expr: {jinja_expr}")
                    if condition_test_result == "False":
                        break
                    del loop_condition_context
                return condition_test_result

            if self.when != []:
                condition_test_result = when(when = self.when, context = {**context,**self.views} )
            else:
                condition_test_result = "True"
            ...
            if condition_test_result == "True":
                ...
                for expr in expressions:
                    ...
                    view_index = 0
                    for var_name,var_data in self.views.items():
                        logging.debug(f"looping views, var_name: {var_name}, var_data: {var_data}")
                        # view_template = Templater(template_string=str(var_data), static_context= {**context,**self.context,**view_context})
                        templated_var = view_templates[view_index].render({**loop_context,**last_view_context},eval_str=True)
                        # templated_var = view_template.render({**loop_context,**view_context},eval_str=True)
                        logging.debug(f"templated_var: {templated_var}")
                        last_view_context = {var_name: templated_var}
                        view_context.update({var_name: templated_var })
                        view_index += 1
                    del last_view_context
                    ...
                    if stylings != []:
                        ...
                        # if styling != []:
                        stylized_value = value
                        for styling_attributes in styling:
                            color_conditions = [color_conditions for color_conditions in  styling_attributes['when'] ]
                            logging.info(f"color_conditions: {color_conditions}")
                            # logging.info(color_conditions)
                            condition_color = when(when = color_conditions, context = {**context,**self.views} )
                            if condition_color == "True":
                                stylized_value = value = Styling().apply(value = value,format=self.format, styling_attributes = styling_attributes)
                            else:
                                stylized_value = value
